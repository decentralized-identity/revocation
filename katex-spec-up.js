module.exports = function (options = {}) {
  ...
  function applyReplacers(doc) {
    return doc.replace(replacerRegex, function (match, type, args) {
      ...
      return replacer
        ? replacer.transform(...args.trim().split(replacerArgsRegex))
        : match;
    });
  }
    ...
    let html = Object.keys(group)
      ...
      .reduce((html, name) => {
        ...
        <dd>
          ...
          ${ref.authors.join("; ")}; ${
          ref.rawDate
        }. <span class="reference-status">Status: ${ref.status}</span>.
        </dd>
      `);
  ...
  try {
    ...
    const specCorpus = fs.readJsonSync(
      modulePath + "/assets/compiled/refs.json"
    );
    ...
    const md = require("markdown-it")({
      ...
      .use(require("spec-up/src/markdown-it-extensions.js"), [
        {
          ...
          parse(token, type, primary) {
            ...
            if (type === "def") {
              return token.info.args.reduce((acc, syn) => {
                return `<span id="term:${syn
                  .replace(spaceRegex, "-")
                  .toLowerCase()}">${acc}</span>`;
              }, primary);
            } else {
              return `<a class="term-reference" href="#term:${primary
                .replace(spaceRegex, "-")
                .toLowerCase()}">${primary}</a>`;
            }
          },
        },
      ])
    ...
    async function render(spec, assets) {
      try {
        ...
        return new Promise(async (resolve, reject) => {
          Promise.all(
            (spec.markdown_paths || ["spec.md"]).map((path) => {
              return fs
                .readFile(spec.spec_directory + path, "utf8")
                .catch((e) => reject(e));
            })
          ).then(async (docs) => {
                ...
                <body features="${Object.keys(features).join(" ")}">
                  ...
                  <main>
                    ...
                    <header id="header" class="panel-header">
                      ...
                      <a id="logo" href="${
                        spec.logo_link ? spec.logo_link : "#_"
                      }">
                        <img src="${spec.logo}" />
                      </a>
    ...
    config.specs.forEach((spec) => {
      ...
      let customAssets = (spec.assets || []).reduce(
        (assets, asset) => {
      ...
      if (options.dev) {
        ...
        assetTags.body =
          assets.body.js
            .map((path) => `<script src="${path}" data-manual></script>`)
            .join("") + customAssets.js.body;
      } else {
        assetTags.head = `
          <style>${fs.readFileSync(
            modulePath + "/assets/compiled/head.css",
            "utf8"
          )}</style>
          ${customAssets.css}
          <script>${fs.readFileSync(
            modulePath + "/assets/compiled/head.js",
            "utf8"
          )}</script>
          ${customAssets.js.head}
        `;
        assetTags.body = `<script>${fs.readFileSync(
          modulePath + "/assets/compiled/body.js",
          "utf8"
        )}</script>
